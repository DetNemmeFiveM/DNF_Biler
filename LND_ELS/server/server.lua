...
local resourceName = "LND_ELS" .. (GetCurrentResourceName() ~= "LND_ELS" and " (" .. GetCurrentResourceName() .. ")" or "")
...
function checkVersion()
	PerformHttpRequest(latestVersionPath, function(err, response, headers)
		...
		if curVersion ~= data.version and curVersion < lVer then
			print("--------------------------------------------------------------------------")
			print(resourceName .. " er forfalden.\nNyeste version: " .. data.version .. "\nAktive Version: " .. curVersion .. "\nOpdater venligst fra MrDaGree Github: https://github.com/MrDaGree/ELS-FiveM")
			print("\nOpdaterings log:\n"..data.changelog)
			print("\n--------------------------------------------------------------------------")
		elseif curVersion > lVer then
			print("Din ELS version " .. resourceName .. " Er ikke en forefundet version, dette er ikke korrekt?")
		else
			print(resourceName .. " Kører nyeste opdatering!")
		end
...
RegisterCommand('els', function(source, args)
	...
	if args[1] == 'version' then
		PerformHttpRequest(latestVersionPath, function(err, response, headers)
			...
			if source > 0 then
				return notify(source, "~r~LND_ELS~s~~n~Version " .. curVerCol(thisVersion) .. curVersion)
			end

			local message
			if curVersion ~= data.version and curVersion < thisVersion then
				message = "Din version er forfalden [ " .. GetCurrentResourceName() .. " ]. Din version [ " .. curVersion .. " ]. Nyeste version: [ " .. data.version .. " ]."
			elseif curVersion > thisVersion then
				message = "Din version har en fejl, den er ikke forefundet i version log?"
			else
				message = "Din version af ELS [ " .. GetCurrentResourceName() .. " ] har nyeste opdatering: [ " .. curVersion .. " ]."
			end

			if message then
				return print("LND_ELS (" .. GetCurrentResourceName() .. "): " .. message)
			end

			return print("LND_ELS (" .. GetCurrentResourceName() .. "): Kan ikke fastslå din version")
		end, "GET", "", {version = 'this'})
	elseif args[1] == 'panel' then
		if source == 0 then return end
		if not args[2] then
			TriggerClientEvent("chat:addMessage", source, {
				args = { "LND_ELS", "Mangler Panel type " },
				color = {13, 161, 200}
			})
			return
		end
	...
	elseif not args[1] or args[1] == 'help' then
		TriggerClientEvent("els:notify", source, "~r~LND_ELS~s~~n~Version " .. curVerCol(data.version) .. curVersion)
		TriggerClientEvent("els:notify", source, "~b~Sub-Commands~s~~n~" .. "~p~panel~s~ - Udpeger panel type, options: " .. table.concat(allowedPanelTypes, ", ") .. "~n~~p~version~s~ - Viser nyeste opdatering.~n~~p~help~s~ - Viser notifikationer.")
	end
end)
...
AddEventHandler("els:playerSpawned", function()
	...
	if curVersion < latestVersion then
		TriggerClientEvent("els:notify", source, "~r~LND_ELS~s~~n~Version er forfaldet, opdater snart!")
	elseif curVersion > latestVersion then
		TriggerClientEvent("els:notify", source, "~o~LND_ELS~s~~n~Version er ikke forefundet! replace med nyeste opdatering")
	else
		return
	end
end)
